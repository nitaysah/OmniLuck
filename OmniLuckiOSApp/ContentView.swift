...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                // Light Celestial Gradient Background
                LinearGradient(
                    colors: [
                    ...
                    VStack(spacing: 25) {
                        ...
                        VStack(alignment: .leading, spacing: 22) {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                if useManualDate {
                                    // Manual Text Input for MM/DD/YYYY
                                    ZStack(alignment: .leading) {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                HStack {
                                    ...
                                    Button(action: {
                                        useNAForTime.toggle()
                                        // When checked, time is not sent (none/empty)
                                        // This prevents daily Kundali report from being populated
                                    }) {
                                        HStack(spacing: 4) {
                        ...
                        Button(action: {
                            ...
                            Task {
                                do {
                                    // 1. Show Loading State (Optional: Add a spinner State)
                                    // 2. Call API
                                    // If "Don't know" is checked, send current time but it will be handled as N/A
                                    let result = try await NetworkService.shared.fetchLuck(
                                        name: name,
                                        dob: dob,
                                        birthTime: useNAForTime ? Date() : birthTime, // Send current time if N/A (backend should handle)
                                        birthPlace: birthPlace,
                                        timeIsNA: useNAForTime // New parameter to indicate time is not provided
                                    )
                                    
                                    // 3. Process Result on Main Thread
                                    await MainActor.run {
                                        isButtonPressed = false
                                        
                                        // Navigate to ResultView with LIVE data
                                        // We need to pass data to navigationDestination somehow
                                        // For simplicity in this structure, we'll store it in a state or pass via binding
                                        // BUT simplest native SwiftUI way:
                                        self.apiResult = result // We need to add this State
